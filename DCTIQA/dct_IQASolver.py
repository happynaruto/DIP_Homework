...
import dct_data_loader
import torch.nn as nn

DEVICE = torch.device("cuda:3") if torch.cuda.is_available() else "cpu"


class dct_IQASolver(object):
    ...
    def __init__(self, config, path, train_idx, test_idx):
        ...
        self.model_dct.conv1 = nn.Conv2d(1, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        ...
        train_loader = dct_data_loader.DataLoader(config.dataset, path, train_idx, config.patch_size, batch_size=config.batch_size, istrain=True)
        test_loader = dct_data_loader.DataLoader(config.dataset, path, test_idx, config.patch_size, istrain=False)
